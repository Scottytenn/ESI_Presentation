...
const float BUZZ_THRESHOLD_CM = 5; 
...
void buzzIfClose(float avg_cm) {
  if (isfinite(avg_cm) && avg_cm > 0 && avg_cm <= BUZZ_THRESHOLD_CM) {
    int freq = 1000;
    tone(buzzerPin, freq, 500);  
  } else if(isfinite(avg_cm) && avg_cm > 0 && avg_cm > BUZZ_THRESHOLD_CM){
    int freq = 200;
...
void servoPulse(int pin, unsigned long hi_us) {
  ...
  unsigned long lo_us = (hi_us < T_TOTAL_US) ? (T_TOTAL_US - hi_us) : 0;
